...
> **注意（文件匯入格式）**：目前的文件匯入服務以 **PDF** 為輸入。Markdown 原始檔
> 保留於 `documents/` 方便檢視與維護，灌入向量庫前請先用下方腳本轉成 PDF。

### 從 Markdown 產生 PDF

執行以下指令，會把四份 SOP Markdown 轉成 PDF：

```bash
python scripts/generate_demo_pdfs.py
```

- 輸出位置：`demo_data/documents/pdf/`（每份 SOP 對應一個同名 `.pdf`）。
- 需要 `reportlab`（已列於 `backend/requirements.txt`）；若尚未安裝請先執行
  `pip install -r backend/requirements.txt`。建議使用專案的虛擬環境，例如
  `backend/.venv/bin/python scripts/generate_demo_pdfs.py`。
- 產生的 PDF 保留標題結構與繁體中文，且文字可被後端的 pypdf 正確抽取（已驗證），
  因此可直接用於 PDF 文件匯入與 RAG 檢索。

> **字型限制與替代方案**：腳本使用 ReportLab 內建的 `MSung-Light`（Adobe-CNS1）
> CID 字型來顯示繁體中文，好處是**不需外掛字型檔**。但少數較舊的 PDF 檢視器若未內建
> 對應的 CJK 字型，畫面上可能以替代字型顯示（文字內容與抽取仍正確）。若需在所有檢視器
> 都完美呈現字型，可改用 pandoc 並指定 CJK 字型手動匯出：
>
> ```bash
