...
Agent orchestrator：依序執行 4 個 incident analysis tools，
組合輸出寫入 incident_analysis / insights / action_items，
並把整個流程記錄到 agent_runs（含每個 tool 一筆 tool_calls）。
"""
...
def analyze_incidents(
    ...
    # 只分析尚未被分析過的 cleaned_records；安全可重複執行（不刪舊資料 - Rule 15）
    analyzed_ids = {
        row[0]
    ...
    try:
        ...
        # 將 classify + severity 結果寫入 incident_analysis
        analyses_created = 0
        ...
        # Tool 4：產生 action items（spec 要求 status="open"）
    ...
    except Exception as exc:  # 防呆：任何未預期錯誤都要寫入 agent_runs，再回 500
